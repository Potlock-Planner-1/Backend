  ...
  Potluck.findById(id)
    .then((potluck) => {
      if (potluck) {
        ...
        res
          .status(404)
          .json({ message: "potluck with the ID does not exists" });
      }
    })
    .catch((error) => {
      console.log(error);
  ...
  Potluck.findItem(id)
    .then((items) => {
      if (items.length) {
        ...
        res
          .status(404)
          .json({ message: "Could not find items for given potluck Id" });
      }
    })
    .catch((err) => {
  ...
  Potluck.findGuest(id)
    .then((guests) => {
      if (guests.length) {
        ...
        res
          .status(404)
          .json({ message: "Could not find guests for given potluck Id" });
      }
    })
    .catch((err) => {
...
router.post("/:id/items", (req, res) => {
  ...
  const { id } = req.params;
  itemData.potluck_id = id;

  Potluck.findById(id)
    .then((potluck) => {
      if (potluck) {
        Potluck.addItem(itemData, id).then((item) => {
          res
            .status(201)
            .json({ item, message: "new item is added to the potluck" });
        });
      } else {
        res
          .status(404)
          .json({ message: "Could not find potluck with given id." });
      }
    })
    .catch((err) => {
...
router.post("/:id/guests", (req, res) => {
  ...
  const { id } = req.params;
  guestData.potluck_id = id;

  Potluck.findById(id)
    .then((potluck) => {
      if (potluck) {
        Potluck.addGuest(guestData, id).then((guest) => {
          res.status(201).json({ guest, message: "new guest is added" });
        });
      } else {
        res
          .status(404)
...
router.put("/:id", (req, res) => {
  ...
  Potluck.findById(id)
    .then((potluck) => {
      if (potluck) {
        Potluck.update(changes, id).then((updatePotluck) => {
